...
import (
	...
	"github.com/masudur-rahman/expense-tracker-bot/infra/database/sql/mock"
...
func initializeSqlDatabaseAndUserRepo(ctl *gomock.Controller) (*mock.MockDatabase, *SQLUserRepository) {
	db := mock.NewMockDatabase(ctl)
	db.EXPECT().Table("user").Return(db).MaxTimes(2)
	ur := NewSQLUserRepository(db, logr.DefaultLogger)
...
func TestNewSQLUserRepository(t *testing.T) {
	...
	db, ur := initializeSqlDatabaseAndUserRepo(ctl)

	assert.NotNil(t, ur)
	assert.Equal(t, db.Table("user"), ur.db)
	assert.Equal(t, logr.DefaultLogger, ur.logger)
}

func TestSQLUserRepository_FindByID(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
...
func TestSQLUserRepository_FindByName(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
...
func TestSQLUserRepository_FindByEmail(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
...
func TestSQLUserRepository_FindUsers(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
